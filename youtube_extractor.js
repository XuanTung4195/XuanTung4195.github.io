...
async function getLoginHeaders() {
  ...
  if (globalInfo.loginHeaders != null) {
    return globalInfo.loginHeaders;
  }
...
async function getYoutubeStreamData(param) {
  ...
  if ((globalInfo?.extractorVersion ?? 1) >= 45) {
    /// skip
  ...
  if (methodName == ExtractName.htmlHls) {
    ...
    playerResponseFuture = getJsonPostResponse(
      "player",
      body,
      loginHeader,
    );
  ...
  if (loadedNextResponse) {
    let bodyNext = getContextBody(true);
    bodyNext["videoId"] = videoId;
    bodyNext["contentCheckOk"] = true;
    bodyNext["racyCheckOk"] = true;
    nextResponseFuture = getJsonPostResponse(
      "next",
      bodyNext,
      loginHeader,
    );
  }
  if (nextResponseFuture != null) {
    nextResponse = await nextResponseFuture;
  }
  if (playerResponseFuture != null && playerResponse == null) {
    playerResponse = await playerResponseFuture;
  }
...
async function getCacheVisitorId() {
  if (youtubeVisitorId == null) {
    await getYoutubeVisitorDataForHls();
  }
  return youtubeVisitorId;
}
...
async function extractJavaScriptPlayerCode() {
  ...
  async function getJsonPostResponse(endpoint, body, header = null, forceVisitorData = false) {
  ...
  async function extractStreamData({
    ...
    html5PoToken, loadedNextResponse,
  }) {
    let ret = {};
    ret.loadedNextResponse = loadedNextResponse;
    ...
    if (hlsManifestUrl) {
      ...
      ret.hlsUrl = getManifestUrl("hls", [response?.streamingData])
    }
...
async function getJsonPlayerHtml5PrefWants({videoId, visitorId, sts}) {
  let iosUserAgent = "com.google.ios.youtube/19.45.4 (iPhone16,2; U; CPU iOS 18_1_0 like Mac OS X;)";
  let visitorData = visitorId ?? (globalInfo?.visitorData ?? globalInfo?.requiredVisitorData)
  let body = {
"context": {
"client": {
